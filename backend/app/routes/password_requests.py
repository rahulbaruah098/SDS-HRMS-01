...
def request_password_change():
    ...
    if existing:
        return jsonify({"message": "You already have a pending password change request"}), 409
    ...
    return jsonify({"message": "Password change request sent to Super Admin"}), 201
...
def list_password_requests():
    ...
    rows = list(db.password_requests.find(q).sort("created_at", -1).limit(500))
